...
import { asUploadDoc} from "@/reduxconfig/actions/userActions";
import {useToast} from "./ui/use-toast";

export const validationSchema = Yup.object().shape({
  title: Yup.string().required("Document title is required"),
  document: Yup.mixed().required("Document file is required").test('fileSize', 'Document size must be less than 3MB', (value:any) => {
    if (value) {
...
export function DocModal({setIsLoading, id}: any) {
  ...
  const sendData = async (values: any) => {
    setIsLoading(true);
    try {
      await dispatch(asUploadDoc(id, values, toast));
      setIsLoading(false);
      setOpen(false);
